...
| 項目                              | 狀態 | 說明                                                           |
|----------------------------------|:----:|----------------------------------------------------------------|
| 支援 Ban/Pick 階段步驟（DRAFT_PHASES） | ✅   | 使用 `step = st.number_input(...)` 對應至動作類型，每步驟過濾推薦名單 |
| 勝率預測整合                        | ⛔   | 尚未整合；`wr_lgb_v4.txt` 可用來計算目前藍／紅陣營勝率（未使用）    |
| 英雄名稱顯示                        | ✅   | 已完成；由 `champs.csv` 對應 ID → 名稱                         |
| 多模型比較                          | ⛔   | 尚未支援；可比較 v2 / v3 預測模型輸出效果                     |

---

## 目標：逐步勝率預測曲線

🎯 效果示意：在 UI 或報表中呈現如下表格
...
2. **實際 Python 程式碼**  
   ...
   def compute_progression_wr(picks_b, picks_r, model):
       ...
       for step in range(1, 11):
           ...
           temp_r = deepcopy(picks_r[:max(0, step - 5)])
           # pad to length 5
           while len(temp_b) < 5: temp_b.append(0)
           while len(temp_r) < 5: temp_r.append(0)
           X_step = pd.DataFrame([{
               **{f"blue{i+1}": temp_b[i] for i in range(5)},
               **{f"red{i+1}":  temp_r[i] for i in range(5)}
           }])
