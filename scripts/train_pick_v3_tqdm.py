...
# -*- coding: utf-8 -*-
"""
train_pick_v3_tqdm.py — 修正版，無 optuna.progress，使用 tqdm 顯示 trial 進度
"""
...
def objective(trial):
    ...
    for tr, va in GroupKFold(3).split(X, y, groups):
        ...
        mdl = lgb.train(params, dtr, num_boost_round=args.num_boost,
                        valid_sets=[dva],
...
print("🚀 Optuna search with tqdm …")
study = optuna.create_study(direction="minimize")
for _ in tqdm(range(args.n_trials), desc="Trial"):
    study.optimize(objective, n_trials=1, timeout=args.timeout)

# Final Train
best = study.best_params | dict(
    objective="multiclass", num_class=len(le.classes_),
    metric="multi_logloss", device_type="cpu" if args.cpu else "gpu",
    force_row_wise=True,
    min_data_in_leaf=max(10, study.best_params.get("min_leaf", 10))
)

try:
    final = lgb.train(best, lgb.Dataset(X, y, categorical_feature=cat_cols),
